...
describe.concurrent("trigger", () => {
  describe("alter", () => {
    ...
    test("replace trigger", async () => {
      const branch = new Trigger({ ...props, enabled: "D" });
    ...
    test("set trigger enabled state", async () => {
      const trigger = new Trigger({ ...props, enabled: "D" });
      const change = new SetTriggerEnabledState({ trigger });

      await assertValidSql(change.serialize());

      expect(change.serialize()).toBe(
        "ALTER TABLE public.test_table DISABLE TRIGGER test_trigger",
      );
    });

    test("set trigger enabled state supports replica and always", async () => {
      const replicaTrigger = new Trigger({ ...props, enabled: "R" });
      const alwaysTrigger = new Trigger({ ...props, enabled: "A" });

      const replicaChange = new SetTriggerEnabledState({
        trigger: replicaTrigger,
      });
      const alwaysChange = new SetTriggerEnabledState({
        trigger: alwaysTrigger,
      });

      await assertValidSql(replicaChange.serialize());
      await assertValidSql(alwaysChange.serialize());

      expect(replicaChange.serialize()).toBe(
        "ALTER TABLE public.test_table ENABLE REPLICA TRIGGER test_trigger",
      );
      expect(alwaysChange.serialize()).toBe(
