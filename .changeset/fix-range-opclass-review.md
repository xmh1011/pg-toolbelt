...
Also require omitted range subtype opclass providers to be `DEFAULT`, preserve explicit public built-in type references when deduping implicit refs, avoid treating set-returning routines as scalar callback providers, ignore implicit built-in type refs in self-reference checks, and require non-ordering opclass search operators to resolve to boolean-returning operators when a local same-signature shadow exists.
